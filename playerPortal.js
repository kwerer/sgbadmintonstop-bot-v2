...
// stage 2
export function formatOfGame(bot, msg, role) {
  bot.sendMessage(msg.chat.id, formatOfGameText(role), {
    parse_mode: "Markdown",
    reply_markup: {
...
export function inputFormatOfGame(bot, msg, role) {
  ...
  bot.sendMessage(msg.chat.id, inputFormatOfPlayText(role, input), { parse_mode: "Markdown" });
  return [4, input];
}
...
export function inputLocationOfGame(bot, msg, role) {
  const input = msg.text.toString();
  bot.sendMessage(msg.chat.id, inputLocationOfGameText(role, input), {
    parse_mode: "Markdown",
    reply_markup: {
...
export function inputLevelOfPlay(bot, msg, role) {
  ...
  bot.sendMessage(msg.chat.id, inputLevelOfPlayText(role, input), { parse_mode: "Markdown" });
  return [stage, input];
}
...
export function inputTimeOfGame(bot, msg, role) {
  const input = msg.text.toString();
  bot.sendMessage(msg.chat.id, inputTimeOfGameText(role, input), { parse_mode: "Markdown" });
  return [7, input];
}
// fees
export function inputFeesOfGame(bot, msg, role) {
  const input = msg.text.toString();
  bot.sendMessage(msg.chat.id, inputFeesText(role, input), { parse_mode: "Markdown" });
  return [10, input];
}
...
export function submitGame(bot, chatId, query, gameFormat, location, levelOfPlay, time, fees, contactInformation, role) {
  ...
  bot.sendMessage(chatId, submissionConfirmationText);
  return 12;
}
