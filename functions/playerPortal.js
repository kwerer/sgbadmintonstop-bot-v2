...
export function formatOfGame(bot, role) {
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, formatOfGameText(role), {
    parse_mode: "Markdown",
    reply_markup: {
...
export function inputFormatOfGame(bot, msg, role) {
  ...
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, inputFormatOfPlayText(role, input), { parse_mode: "Markdown" });
  return [4, input];
}
...
export function inputLocationOfGame(bot, msg, role) {
  const input = msg.text.toString();
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, inputLocationOfGameText(role, input), {
    parse_mode: "Markdown",
    reply_markup: {
...
export function inputLevelOfPlay(bot, msg, role) {
  ...
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, inputLevelOfPlayText(role, input), { parse_mode: "Markdown" });
  return [stage, input];
}
...
export function inputTimeOfGame(bot, msg, role) {
  const input = msg.text.toString();
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, inputTimeOfGameText(role, input), { parse_mode: "Markdown" });
  return [7, input];
}
// fees
export function inputFeesOfGame(bot, msg, role) {
  const input = msg.text.toString();
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, inputFeesText(role, input), { parse_mode: "Markdown" });
  return [10, input];
}
...
export function submitGame(bot, query, gameFormat, location, levelOfPlay, time, fees, contactInformation, role) {
  ...
  bot.sendMessage(process.env.LOCAL_BOT_CHAT_ID, submissionConfirmationText);
  return 12;
}
